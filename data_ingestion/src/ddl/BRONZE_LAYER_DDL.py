...
# MAGIC %md
# MAGIC # Bronze Layer DDL — Table Definitions
# MAGIC
# MAGIC | Field | Value |
# MAGIC |-------|-------|
...
# MAGIC | **Version** | 1.0 |
# MAGIC
# MAGIC **Description:** Creates all Bronze layer Delta tables for the FAS Advantage ingestion pipeline. Each table is created using `CREATE OR REPLACE TABLE` with Delta format, liquid clustering (`CLUSTER BY`), and column mapping enabled.
# MAGIC
...
# MAGIC | 4 | `catalog_832` | 63 | vend_id, contract_num, attributes_flag |
...
# MAGIC | 9 | `price_discount` | 18 | vend_id, contract_num, mfr_part, zone, dollar_qty, seq, sale |
...
# MAGIC
# MAGIC > Run this notebook once per environment to provision Bronze layer tables before ingestion.
...
# MAGIC %md
# MAGIC ## Prerequisite: Create Catalog & Schema
# MAGIC Creates the Unity Catalog and schema if they do not already exist. This is idempotent — safe to re-run.

# COMMAND ----------

# MAGIC %skip
# MAGIC # Create catalog if it does not exist
# MAGIC spark.sql(f"CREATE CATALOG IF NOT EXISTS {catalog}")
# MAGIC print(f"[INFO] Catalog '{catalog}' ensured.")
...
# MAGIC print(f"[INFO] Schema '{catalog}.{schema}' ensured.")
...
# MAGIC %md
# MAGIC ## DDL: `bpa_item`
# MAGIC Blanket Purchase Agreement line items. Each row is a product/service line linked to a BPA header.
# MAGIC
# MAGIC | FK Column | References |
# MAGIC |-----------|------------|
# MAGIC | `bpa_id` | `bpa_header.id` |
# MAGIC | `status` | `bpa_status.status_id` |
# MAGIC | `pricing_program` | `bpa_pricing_program.program_id` |
...
# MAGIC %md
# MAGIC ## DDL: `bpa_item_price`
# MAGIC Tiered pricing for BPA items. Each row represents a quantity range and its associated discount/price.
...
# MAGIC %md
# MAGIC ## DDL: `catalog_832`
# MAGIC EDI 832 catalog records — vendor/contract pricing metadata including delivery terms, discounts, and business attribute flags.
...
# MAGIC %md
# MAGIC ## DDL: `contract_zone`
# MAGIC Maps contracts to geographic zones (state-level). Used to determine zone-based pricing.
# MAGIC
# MAGIC **Primary Key:** `vend_id`, `contract_num`, `state`
...
# MAGIC %md
# MAGIC ## DDL: `gsin_hide_remove`
# MAGIC Tracks GSINs (Global Standard Identification Numbers) that have been hidden or removed from display, along with the requester and reason.
...
# MAGIC %md
# MAGIC ## DDL: `gsin_hide_remove_hist`
# MAGIC Audit history of GSIN hide/remove actions — preserves the full timeline of status changes per GSIN and contract item.
...
# MAGIC %md
# MAGIC ## DDL: `mp_product`
# MAGIC Marketplace product catalog. Each row is a unique product identified by GSIN, with manufacturer details, UNSPSC classification, and delivery info.
# MAGIC
# MAGIC **Primary Key:** `gsin`
...
# MAGIC %md
# MAGIC ## DDL: `price_discount`
# MAGIC Quantity-based price discounts per vendor/contract/part/zone. Supports tiered pricing with quantity ranges and discount percentages.
...
# MAGIC ## DDL: `sin_limit`
# MAGIC
# MAGIC Special Item Number (SIN) order limits per vendor/contract. Defines minimum and maximum order values and delivery descriptions.**Primary Key:** `vend_id`, `contract_num`, `spec_item`
# MAGIC
...
# MAGIC %md
# MAGIC ## DDL: `suspend_contract`
# MAGIC Contracts that are currently suspended. The `suspend_type` indicates the reason category for suspension.
...
# MAGIC %md
# MAGIC ## DDL: `zone_price`
# MAGIC Zone-based unit pricing per vendor/contract/part. Includes list price, schedule price, sale pricing with date ranges, and project/location codes.
...
# MAGIC %md
# MAGIC ## DDL: `contracts`
# MAGIC Master contract records — vendor details, socioeconomic indicators, schedule/SIN assignments, and contract lifecycle dates. One of the largest reference tables in the Bronze layer (60 columns).
...
# MAGIC %md
# MAGIC ## DDL: `sin`
# MAGIC Schedule-level Special Item Number (SIN) definitions. Contains SIN metadata, contracting officer contact info, and display flags for eLibrary/eBuy.
...
# MAGIC %md
# MAGIC ## DDL: `adv_product`
# MAGIC Advantage product catalog table. Contains product details, pricing, vendor info, environmental compliance flags, delivery codes, and business attribute metadata from GSA Advantage.
...
# MAGIC %md
# MAGIC ## DDL: `item_xref`
# MAGIC Item cross-reference table. Maps items to contracts, schedules, SINs, NSNs, vendors, and manufacturers. Includes pricing, environmental compliance flags, FOB codes, and business attribute metadata.
...
# MAGIC %md
# MAGIC ## DDL: `product_file`
# MAGIC Product catalog file table. Contains detailed product attributes including dimensions, weights, packaging, warranty info, pricing terms, FOB codes, environmental compliance flags, and service indicators (options, accessories, maintenance, lease, rental, e-warranty).
...
# MAGIC %md
# MAGIC ## DDL: `order_status`
# MAGIC Order status tracking table. Captures shipment status per order line — including contract, PO number, NSN/manufacturer part, status date, quantity, shipping mode, tracking number, and estimated ship date.
# MAGIC
# MAGIC **Indexed columns (source):** `po_number_reqn_num`, `awd_unique_id` + `line_num`, `bv_order_num` + `nsn_mfr_part` + `line_status`, `date_created`
...
# MAGIC %md
# MAGIC ## DDL: `item_xref_attributes`
# MAGIC Extended attributes for item cross-reference records. Stores 508 compliance URLs, scan codes, true manufacturer part, PSC code, CAGE code, and EULA indicator per contract/item/manufacturer combination.
# MAGIC
# MAGIC **Unique index (source):** `contract_num`, `item_num`, `mfr_name`
